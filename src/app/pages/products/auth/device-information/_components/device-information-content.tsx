...
export function DeviceInformationContent() {
  const { isLoading, error, data, getData } = useVisitorData(
    { extendedResult: true },
    { immediate: false } // Cambiar a false para mejor control de errores
  );
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Solo mostrar error si no es un error de red esperado (modo offline)
        const isNetworkError = err?.message?.includes("Network") || 
                               err?.message?.includes("ERR_NAME_NOT_RESOLVED") ||
                               err?.message?.includes("Failed to load");
        
        if (isNetworkError) {
          // Silenciar errores de red en consola, pero mostrar mensaje amigable al usuario
          setLoadError("FingerprintJS service is unavailable. The page will work with previously saved events.");
        } else {
          console.error("Failed to fetch visitor data on mount:", err);
          setLoadError(err?.message || "Failed to load FingerprintJS agent.");
        }
      }
    };
  ...
  return (
    <div className="mt-6 space-y-6">
      ...
      {/* Error State - Solo mostrar si hay eventos guardados o si es un error crítico */}
      {(error || loadError) && (
        <div className="mb-6 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400">
          <div className="flex items-start gap-2">
            <svg className="h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <div className="flex-1">
              <p className="font-medium">FingerprintJS Service Unavailable</p>
              <p className="mt-1 text-xs">
                {loadError || error?.message || "Unable to connect to FingerprintJS service. This may be due to network connectivity or DNS resolution issues."}
              </p>
              <p className="mt-2 text-xs opacity-75">
                <strong>Note:</strong> The page will continue to work with previously saved events. You can view and interact with existing device information data.
              </p>
              {events.length > 0 && (
                <p className="mt-2 text-xs font-medium text-amber-900 dark:text-amber-300">
                  ✓ {events.length} saved event{events.length !== 1 ? 's' : ''} available
                </p>
              )}
              <button
                onClick={async () => {
                  try {
                    ...
                    setLoadError("Service still unavailable. Please check your network connection.");
                  }
                }}
                className="mt-3 rounded bg-amber-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-amber-700"
              >
                Retry Connection
              </button>
            </div>
