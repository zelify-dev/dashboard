...
export function SyntaxHighlightTextarea({
  ...
  const sharedMinHeight = "min-h-[600px]";
...
function highlightHtml(content: string) {
  ...
  escaped = escaped.replace(/(&lt;\/?)([\w:-]+)([^&]*?)(\/?&gt;)/g, (_match, open, tagName, attrs, close) => {
    let attrText = attrs;
    attrText = attrText.replace(
      /([\s/]+)([\w:-]+)(\s*=\s*)(?:(&quot;.*?&quot;)|(&#39;.*?&#39;))/g,
      (_m, leading, attrName, equal, doubleQuoted, singleQuoted) => {
    ...
    attrText = attrText.replace(/([\s/]+)([\w:-]+)/g, (_m, leading, attrName) => {
  ...
  escaped = escaped.replace(/\$\{[^}]+\}/g, (match) => `<span class="text-pink-300">${match}</span>`);
  return escaped;
}
